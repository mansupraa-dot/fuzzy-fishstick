...
- **Тесты:** Vitest + @testing-library/react, jsdom. Команда: `npm run test:run`. 84 теста, все зелёные. Pool: `forks` (в vite.config.js — фикс race condition при 14+ файлах).
...
- **Данные:** `src/data/products.js` — 19 SKU, экспорты: `PRODUCTS`, `CATEGORY_LABELS`, `SUBCATEGORIES`, `getProductById(id)`, `getProductsByCategory(cat, sub)`, `sortProducts(products, sort)`. `src/data/portfolio.js` — 6 проектов, экспорты: `PROJECTS`, `getProjectBySlug(slug)`
