...
[Plan 4: Lead-gen/Content]  ✅ DONE — 2026-05-20, 9 коммитов, 84 теста
[Plan 5: Home]              ✅ DONE — 2026-05-20, 4 коммита, 92 теста
[Plan 6: ???]              ← МЫ ЗДЕСЬ — следующее
```

**Следующее действие:** определить Plan 6. Использовать `superpowers:brainstorming`.
...
- **Тесты:** Vitest + @testing-library/react, jsdom. Команда: `npm run test:run`. 103 теста, все зелёные. Pool: `forks` (в vite.config.js — фикс race condition при 14+ файлах).
...
- **Данные:** `src/data/products.js` — 19 SKU, экспорты: `PRODUCTS`, `CATEGORY_LABELS`, `SUBCATEGORIES`, `getProductById(id)`, `getProductsByCategory(cat, sub)`, `sortProducts(products, sort)`. `src/data/portfolio.js` — 6 проектов, экспорты: `PROJECTS`, `getProjectBySlug(slug)`. `src/data/collections.js` — 3 коллекции (Скандинавский, Минимализм, Japandi), экспорты: `COLLECTIONS`, `getCollectionBySlug(slug)`
...
**Выбрано:**
- Имя бренда: **Archittell** («архитектура» + «рассказчик»). В коде заменено везде (2026-05-22).

**Ещё не выбрано:**
- Домен
- Акцентный цвет (TBD после логотипа; сейчас `--color-accent: #4F6AF0` как заглушка)
- Лого, tone of voice
- Реальные контакты (адрес шоурума, телефон, email, Telegram) — в ContactPage/ShowroomPage стоят заглушки
...
- Plan 4 (Lead-gen + Content): выполнен, 84 теста — ServicesPage/ProcessPage/PhilosophyPage (Light Glass), PortfolioPage + PortfolioItemPage (slug + статус), BriefPage (лид-форма), ShowroomPage (запись на визит)
- Plan 5 (Home): выполнен, 92 теста — HowWeWork/Portfolio/CTA (Light Glass rebuild), HomePage rebuild (категории glass-card, удалены Philosophy + popular products)
- Plan 6 (Collections + Delivery + Return): выполнен, 103 теста — CollectionsPage, CollectionItemPage (grouped by category), DeliveryPage, ReturnPage. Все PlaceholderPage закрыты кроме /search.
